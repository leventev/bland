...
mat: union(enum) {
    dc: matrix.Matrix(FloatType),
    ac: complex_matrix.ComplexMatrix(FloatType),
},
...
pub fn init(
    ...
    if (complex) {
        return MNA{
            .mat = .{
                .ac = try complex_matrix.ComplexMatrix(FloatType).init(
                    allocator,
        ...
        return MNA{
            .mat = .{
                .dc = try matrix.Matrix(FloatType).init(
                    allocator,
                    total_variable_count,
                    total_variable_count + 1,
                ),
        ...
        switch (self.values) {
            ...
            .ac => |res| {
                for (res.voltages, 0..) |z, idx| {
                    ...
                    complex_matrix.prettyPrintComplex(FloatType, z);
                    std.debug.print("\n", .{});
                }
